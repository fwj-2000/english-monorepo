...
### refreshTokenApi 网络错误处理

#### 问题背景

在调用 `refreshTokenApi` 时，可能会遇到网络错误，例如：
...
- 状态无法正确清理（`isRefreshing` 一直为 `true`）
- 后续请求都无法正常刷新 Token
- `requestQueue` 中的请求永远无法被执行

#### 解决方案

使用 `try-catch-finally` 结构来确保无论成功或失败，状态都能被正确清理：
...
| --------- | -------------------------------------------------------------------------------------------------- |
| `try`     | 尝试调用刷新接口                                                                                   |
| `catch`   | 捕获网络错误，将错误传递给调用方                                                                   |
| `finally` | **最重要**！无论成功或失败，都会执行：<br>1. 清空 `requestQueue`<br>2. 重置 `isRefreshing = false` |

#### 为什么 finally 很重要？

即使刷新接口因为网络问题失败了，`finally` 块也会执行，确保：

- `isRefreshing` 被重置为 `false`，下次遇到 401 时可以再次尝试刷新
- `requestQueue` 被清空，避免内存泄漏

虽然网络错误时队列中的请求会失败，但至少系统可以恢复正常状态，用户可以重新尝试！
