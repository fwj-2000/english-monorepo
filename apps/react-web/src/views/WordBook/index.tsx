    ...
    <div className='max-w-6xl mx-auto mt-10 rounded-2xl p-10 bg-gray-50 border border-gray-200'>
      ...
      <div className='flex items-center flex-wrap gap-2 mb-8'>
        <Input
          ...
          placeholder='请输入单词'
          value={query.word}
          onChange={e => setQuery(p => ({ ...p, word: e.target.value }))}
          onPressEnter={searchWord}
        />
        {tags.map(({ k, l }) => (
          <Checkbox key={k} checked={!!query[k]} onChange={() => toggleCheck(k)}>
            {l}
          </Checkbox>
        ))}
        <Button type='primary' onClick={searchWord}>
          搜索
        </Button>
      </div>
        ...
        {list.map(item => (
          <div key={item.id} className='card card-hover p-4 cursor-pointer'>
            ...
            <div className='text-sm text-gray-400 mb-1 flex items-center gap-2'>
              {item.phonetic}
              <PlayCircleOutlined onClick={() => playAudio(item.word)} className='cursor-pointer' />
            </div>
            <div className='text-sm text-gray-600 mb-1 line-clamp-2'>{item.definition}</div>
            <div
              className='text-sm text-gray-400 mb-1 line-clamp-2'
      ...
      {total > query.pageSize && (
        <Pagination
          ...
          total={total}
          onChange={(page, size) => {
            const nq = { ...query, page, pageSize: size }
            setQuery(nq)
            getList(nq)
          }}
        />
