...
export function Course() {
  ...
  useEffect(() => { getList(currentTab) }, [currentTab])
  ...
  return (
    <div className="max-w-6xl mx-auto pt-12 pb-24">
      <header className="mb-12 text-center">
        ...
        <p className="mt-3 text-gray-500 text-sm max-w-md mx-auto">一次购买，长期有效 · 覆盖高考、考研、四六级、托福雅思等</p>
      </header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {list.map((item) => (
          <article key={item.id} className="group bg-white rounded-xl border border-gray-200 overflow-hidden flex flex-col">
            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <p className="mt-2 text-sm text-gray-500 line-clamp-2 leading-relaxed flex-1">{item.description}</p>
      ...
      <PayDialog visible={payVisible} course={selectedCourse} onClose={() => setPayVisible(false)} />
    </div>
  )
}
